<script>
  ...
  import { APIDepartmentsCreate } from "../../../services/api/departments/Create.js";
  ...
  export default {
    ...
    methods: {
      ...
      formIsValid() {
        if (this.form.name === "") {
          this.msg = "Пожалуйста, введите название отдела"
          return false
        }
        this.msg = ""
        return true
      ...
      async createDepartment() {
        if (this.formIsValid()) {
          ...
          let response = await APIDepartmentsCreate(this.form)

          if (response.isOk) {
            this.redirectToDepartment(response.data)
          } else {
