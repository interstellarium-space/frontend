<script>
  ...
  import { APIContractsCreate } from "../../../services/api/contracts/Create.js";
  ...
  export default {
    ...
    methods: {
      ...
      formIsValid() {
        if (this.form.name === "") {
          this.msg = "Пожалуйста, введите название контракта"
          return false
        }
        this.msg = ""
        return true
      ...
      async createContract() {
        if (this.formIsValid()) {
          ...
          let response = await APIContractsCreate(this.form)

          if (response.isOk) {
            this.redirectToContract(response.data)
          } else {
