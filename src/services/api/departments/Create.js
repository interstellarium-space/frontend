...
export async function APIDepartmentsCreate(departmentData) {
    let request = prepareAPIRequest("/api/departments")
    let response = {}
...
function handleError(error) {
    ...
    if (error.response) {
        switch (error.response.status) {
            case 400: data.msg = "Отдел с таким названием уже есть"; break
