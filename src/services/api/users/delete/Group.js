...
export async function APIUsersDeleteGroup(userId, groupId) {
    let request = prepareAPIRequest(`/api/users/${userId}/group/${groupId}`)
    let response = {}

    try {
        response = await axios.delete(request.url, request.config)
    } catch (error) {
