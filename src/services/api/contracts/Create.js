...
export async function APIContractsCreate(contractData) {
    let request = prepareAPIRequest("/api/contracts")
    let response = {}
    ...
    try {
        response = await axios.post(request.url, {
            ...
            finish_date: contractData.finishDate
        }, request.config)
    } catch (error) {
...
function handleError(error) {
    ...
    if (error.response) {
        switch (error.response.status) {
            case 400: data.msg = "Такой проект уже есть"; break
