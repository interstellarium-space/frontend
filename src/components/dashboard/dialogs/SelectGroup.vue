    ...
    methods: {
      ...
      async search() {
        if (this.searchFormIsValid()) {
          ...
          let response = await APIGroupsGet(this.searchFilters)
...
<template>
  <Dialog :id="this.id">
    ...
    <template v-slot:dialog-body>
      ...
      <div v-for="group in this.groups" class="interstellarium-unit-card">
        <a @click="this.select(group)" data-bs-dismiss="modal" class="interstellarium-unit-link">
          {{ group.name }}
        </a>
      </div>
    </template>
